...
INPUT ENUM_PP_TYPE Rider_Trend_Pivot_Type = PP_CAMARILLA;  // Pivot type for trend calculation
...
class Stg_Rider : public Strategy {
 ...
 public:
  ...
  bool IsTrend(ENUM_ORDER_TYPE _cmd) {
    ...
    bool _result = false;
    double _tvalue = TrendStrength(::Rider_Trend_Tf, Get<ENUM_STRATEGY_PARAM>(STRAT_PARAM_SHIFT));
    switch (_cmd) {
      case ORDER_TYPE_BUY:
  ...
  virtual bool SignalOpenFilterMethod(ENUM_ORDER_TYPE _cmd, int _method = 0) {
    bool _result = Strategy::SignalOpenFilterMethod(_cmd, _method) && IsTrend(_cmd);
    return _result;
  }
